...
## Algorithm
1. Input the Data Read the number of gardens n and the number of paths m. Read each of the m paths that connect two gardens and store them in an adjacency list (undirected graph).
2. Build the Adjacency List For each path (x, y), add y to the adjacency list of x and vice versa (convert to 0-based indexing).
3. Initialize Flower Assignment Create an array ans[] of size n to store the flower type (1–4) for each garden. Each garden will eventually have one flower type assigned.
4. Assign Flowers Greedily For each garden: Create a boolean array used[5] to track which flower types are already used by its adjacent gardens. For every neighbor, mark its flower type as used.
...
<img width="518" height="491" alt="image" src="https://github.com/user-attachments/assets/c07589f0-ee4e-4d9c-b7ad-d8e000a31c35" />
